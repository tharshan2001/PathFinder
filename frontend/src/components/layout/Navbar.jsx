...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-[#FEE2E2]">
      <div className="container mx-auto max-w-[1200px] px-4">
        <div className="flex items-center justify-between h-[60px]">
          {/* Left section - Logo and search */}
          <div className="flex items-center gap-4">
            <Link to="/" className="flex items-center gap-2">
              <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-[#B91C1C] to-[#991B1B] flex items-center justify-center">
            ...
            <div className="relative hidden md:block">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-[#334155]" size={18} />
              <input
                type="text"
              ...
              <Link
                ...
                className={`flex flex-col items-center justify-center px-4 h-[60px] min-w-[70px] border-b-2 transition-colors ${
                  isActive(item.path)
                    ...
                    : 'border-transparent text-[#334155] hover:text-[#B91C1C]'
                }`}
              >
            ...
            <div className="relative ml-2">
              ...
              >
                <img
                  src={user.avatar}
                  alt={user.name}
                  className="w-7 h-7 rounded-full border-2 border-[#B91C1C]"
                />
              ...
              {showUserMenu && (
                <div className="absolute right-0 mt-0 w-[280px] dropdown p-0 animate-fadeIn">
                  ...
                  <div className="p-4 border-b border-[#E2E8F0]">
                    <div className="flex items-start gap-3">
                      <img
                        src={user.avatar}
                        alt={user.name}
                        className="w-14 h-14 rounded-full border-2 border-[#B91C1C]"
                      />
                      <div>
                        <p className="font-semibold text-[#0F172A]">{user.name}</p>
                        <p className="text-sm text-[#334155]">Software Developer</p>
                      </div>
                    </div>
                    <Link
                      to="/profile"
                  ...
                  <div className="py-2 border-b border-[#E2E8F0]">
                    <p className="px-4 py-1 text-xs font-semibold text-[#334155] uppercase tracking-wide">Account</p>
                    <Link
                      to="/saved"
                      className="block px-4 py-2 text-sm text-[#0F172A] hover:bg-[#FEE2E2] hover:text-[#B91C1C]"
                      onClick={() => setShowUserMenu(false)}
                    >
                      Saved Items
                    </Link>
                    ...
                    <Link
                      to="/recommendations"
                      className="flex items-center gap-2 px-4 py-2 text-sm text-[#0F172A] hover:bg-[#FEE2E2] hover:text-[#B91C1C]"
                      onClick={() => setShowUserMenu(false)}
                    >
                  ...
                  <div className="py-2">
                    <button
                      className="w-full text-left px-4 py-2 text-sm text-[#0F172A] hover:bg-[#FEE2E2] hover:text-[#B91C1C]"
                      onClick={() => setShowUserMenu(false)}
                    >
                      Sign Out
                    </button>
