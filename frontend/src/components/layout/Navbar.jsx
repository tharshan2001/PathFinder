...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-[var(--slate-300)]">
        ...
        <div className="flex items-center justify-between h-[60px]">
          ...
          <div className="flex items-center gap-4">
            <Link to="/" className="flex items-center gap-2">
              ...
              <span className="text-xl font-bold text-[var(--slate-900)] hidden md:block">PathFinder</span>
            </Link>
              ...
              <Link
                ...
                className={`flex flex-col items-center justify-center px-4 h-[60px] min-w-[70px] border-b-2 transition-colors ${
                  isActive(item.path)
                    ...
                    : 'border-transparent text-[var(--slate-500)] hover:text-[var(--primary)]'
                }`}
              >
            ...
            <div className="relative ml-2">
              ...
              >
                <img
                  src={user.avatar}
                  alt={user.name}
                  className="w-7 h-7 rounded-full border-2 border-[var(--primary)]"
                />
              ...
              {showUserMenu && (
                <div className="absolute right-0 mt-0 w-[280px] dropdown p-0 animate-fadeIn">
                  {/* User info */}
                  <div className="p-4 border-b border-[var(--slate-300)]">
                    <div className="flex items-start gap-3">
                      ...
                      />
                      <div>
                        <p className="font-semibold text-[var(--slate-900)]">{user.name}</p>
                        <p className="text-sm text-[var(--slate-500)]">Software Developer</p>
                      </div>
                    </div>
                    <Link
                      to="/profile"
                  ...
                  {/* Account section */}
                  <div className="py-2 border-b border-[var(--slate-300)]">
                    <p className="px-4 py-1 text-xs font-semibold text-[var(--slate-500)] uppercase tracking-wide">Account</p>
                    <Link
                      to="/saved"
                      className="block px-4 py-2 text-sm text-[var(--slate-900)] hover:bg-[var(--primary-light)] hover:text-[var(--primary)]"
                      onClick={() => setShowUserMenu(false)}
                    >
                      Saved Items
                    </Link>
                    ...
                    <Link
                      to="/recommendations"
                      className="flex items-center gap-2 px-4 py-2 text-sm text-[var(--slate-900)] hover:bg-[var(--primary-light)] hover:text-[var(--primary)]"
                      onClick={() => setShowUserMenu(false)}
                    >
                  ...
                  <div className="py-2">
                    <button
                      className="w-full text-left px-4 py-2 text-sm text-[var(--slate-900)] hover:bg-[var(--primary-light)] hover:text-[var(--primary)]"
                      onClick={() => setShowUserMenu(false)}
                    >
                      Sign Out
                    </button>
